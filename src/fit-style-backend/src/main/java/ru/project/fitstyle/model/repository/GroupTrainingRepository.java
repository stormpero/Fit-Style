...
public interface GroupTrainingRepository extends JpaRepository<GroupTraining, Long> {
    @Query("select new ru.project.fitstyle.model.dto.training.GroupTrainingDto(v.id, v.startDate, v.endDate, v.status, w.id, w.name, w.surname, w.patronymic, v.trainingType.name) " +
    ...
    @Query("select new ru.project.fitstyle.model.dto.training.GroupTrainingDto(v.id, v.startDate, v.endDate, v.status, t.id, t.name, t.surname, t.patronymic, v.trainingType.name) " +
